...
do
  case $1 in
    ...
    *)
      ...
      echo "Usage: $0 -s|--server-name your-server-name [-g|--generate-ca-certs] -c|--client-name your-client-certname"
      exit
      ;;
  esac
  shift  
...
if [ "$THERE_IS_JOB" != "true" ] 
then 
  echo "You have not specified any of server or client certificate to generate."
  exit
fi
...
then
	...
	chmod 600 $ROOT_HOME_FOLDER/private/ca.key.pem
	...
	chmod 644 $ROOT_HOME_FOLDER/certs/ca.cert.pem
	...
	chmod 600 $INTER_HOME_FOLDER/private/intermediate.key.pem
	...
	chmod 644 $INTER_HOME_FOLDER/certs/intermediate.cert.pem
	...
	chmod 644 $INTER_HOME_FOLDER/certs/ca-chain.cert.pem
fi
...
then
  ...
  chmod 600 $SERVER_HOME_FOLDER/private/"$SERVERCN".key.pem
...
then
  ...
  chmod 600 $CLIENT_HOME_FOLDER/private/"$CLIENTCN".key.pem
  ...
  chmod 644 $CLIENT_HOME_FOLDER/certs/"$CLIENTCN".cert.pem
  ...
  chmod 600 $CLIENT_HOME_FOLDER/certs/"$CLIENTCN".client.cert.pem
fi
